...
class Technicals():
    ...
    def process_candles(self, df):
        ...
        log_cols = ['time', 'volume', 'mid_c', 'SPREAD', 'PAIR', short_col, long_col, long_prev, 'D_PREV', 'D_NOW' ]
        self.log_message(f"Process_df\n{df[log_cols].tail(2)}")
        self.log_message(f"Trade decision: {decision}")
        self.log_message("")
